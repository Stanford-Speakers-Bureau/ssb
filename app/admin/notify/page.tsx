...
export default function AdminNotifyPage() {
  const [events, setEvents] = useState<EventWithNotifications[]>([]);
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-blue-400">
                {events.reduce((acc, e) => acc + e.notifications.length, 0)}
              </p>
            </div>
            <div>
              <p className="text-zinc-500 text-sm">Avg per Event</p>
              <p className="text-2xl font-bold text-emerald-400">
                {(events.reduce((acc, e) => acc + e.notifications.length, 0) / events.length || 0).toFixed(1)}
              </p>
            </div>
          </div>
        </div>
