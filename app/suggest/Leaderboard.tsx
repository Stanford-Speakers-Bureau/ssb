    ...
    try {
      ...
      if (!response.ok) {
        if (data.alreadyVoted) {
          ...
          setSuggestions(prev => 
            prev.map(s => s.id === speakerId ? { ...s, hasVoted: true } : s)
          );
        } else {
        ...
        {suggestions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-xs">
              Be the first to suggest a speaker you&apos;d like to see at Stanford!
            </p>
          </div>
                ...
                >
                  {votingId === suggestion.id ? (
                    ...
                    <>
                      ...
                      <span className={`${hoveredId === suggestion.id ? 'hidden lg:inline' : ''}`}>Voted</span>
                      <span className={`${hoveredId === suggestion.id ? 'lg:hidden' : 'hidden'}`}>Unvote</span>
                    </>
                  ) : (
                    <>
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
