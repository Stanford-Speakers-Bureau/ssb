...
export default function TicketButton(props: TicketButtonProps) {
  ...
  const ticketTypeLabel = actions.ticketType
    ? actions.ticketType.charAt(0).toUpperCase() +
      actions.ticketType.slice(1).toLowerCase()
    : null;
  ...
  const cancelTitle = actions.initialIsScanned
    ...
    : "Cancel Ticket?";
  const boldType = ticketTypeLabel?.toUpperCase() ? (
    <strong className="text-zinc-200">{ticketTypeLabel.toUpperCase()}</strong>
  ) : null;
  const boldEvent = actions.eventName ? (
    <strong className="text-zinc-200">{actions.eventName}</strong>
  ) : null;
  const boldName = cancelTicketOwnerName ? (
    <strong className="text-zinc-200">{cancelTicketOwnerName}</strong>
  ) : null;

  const cancelDescription = actions.initialIsScanned ? (
    <>
      The 1 {boldType} ticket{boldEvent && <> to {boldEvent}</>}
      {boldName && <> for {boldName}</>} has already been scanned at the door
      and can no longer be cancelled.
    </>
  ) : (
    <>
      Are you sure you want to cancel 1 {boldType} ticket
      {boldEvent && <> to {boldEvent}</>}
      {boldName && <> for {boldName}</>}? You may not be able to get it back if
      you cancel.
    </>
  );
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        onConfirm={
          actions.initialIsScanned ? undefined : actions.handleCancelTicket
        }
      />
