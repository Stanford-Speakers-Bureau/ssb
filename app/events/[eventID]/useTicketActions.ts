...
const DEFAULT_INELIGIBLE_MESSAGE =
  "According to ASSU records, you are ineligible for online ticketing as your student activity fee for StanfordSpeakers Bureau has been waived. Please contact ASSU for further details. We encourage you to show up to the venue early to join the standby line instead.";
...
export default function useTicketActions({
  ...
  const [ineligibleFaqUrl, setIneligibleFaqUrl] =
    useState(DEFAULT_ASSU_FAQ_URL);
  ...
  const showIneligibleModalForResponse = useCallback(
    (
      ...
      const isRoleRestrictionResponse = data?.code === ROLE_INELIGIBLE_CODE;
  ...
  const reconcileWaitlistStatus =
    useCallback(async (): Promise<WaitlistStatusCache | null> => {
      try {
        const response = await fetchWithTimeout(
  ...
  }, [eventId, eventName, reconcileWaitlistStatus]);
  ...
  }, [eventId, eventName, referralsEnabled, validateReferralCode]);
  ...
  const isButtonDisabled =
    isLoading || isSalesDisabled || (!!referralWarning && !hasTicket);
