...
type SignInToVoteModalProps = {
  open: { id: string; speaker: string } | null;
  onClose: () => void;
};

export default function SignInToVoteModal({
  open,
  onClose,
}: SignInToVoteModalProps) {
  useEffect(() => {
    if (!open) return;
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="signin-to-vote-title"
          >
            <button
              type="button"
              onClick={onClose}
            ...
            <h2
              id="signin-to-vote-title"
              className="mt-2 font-serif text-2xl text-white leading-tight"
            >
              Sign in to vote for {open.speaker}
            </h2>
            <p className="mt-3 font-sans text-sm text-zinc-400 leading-relaxed">
              We require a Stanford sign-in for votes to keep the leaderboard
              honest and prevent spam. It only takes a few seconds, and
              we&rsquo;ll bring you right back to cast your vote.
            </p>
