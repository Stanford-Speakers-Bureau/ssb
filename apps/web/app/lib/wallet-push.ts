 ...
 *
 * Mirrors admin's pushWalletUpdate so the web app (where scanning happens) can
 * push live pass updates too. APNs needs HTTP/2 + mTLS, which the Worker can't
 * do — so the actual push runs in the Supabase edge function; we just invoke it.
 *
 * Best-effort: this never throws into the calling request. A failed or
 * unconfigured push must not fail the scan — the pass will still pick up the
 * change the next time the device polls the web service.
 */
